...
A dedicated Warrantee-only Cloud Run deployment remains specified and tested in `services/clamav-scanner/cloud-run-config.mjs` as an unactivated future option. For the early launch, the owner selected the existing DigitalOcean Droplet and approved logical container isolation. The active deployment procedure is in `services/clamav-scanner/DIGITALOCEAN.md`. It defines a separate Compose project, private network, signature volume, root-only environment, host-loopback wrapper port, dedicated hostname, resource limits, and no document persistence or mounts from other workloads. The application supports a bounded `DOCUMENT_SECURITY_SCANNER_TIMEOUT_MS` for remote scanner verification.

The Google Cloud project displayed `Start your Free Trial` on 5 September 2026. No API, service, repository, secret, or billing relationship was created, and the owner decided not to activate it. The current internal scanner must remain configured until the DigitalOcean ClamAV runtime passes health, clean-file, isolated EICAR, log-privacy, shared-host regression, CI, and Production Security Gate checks.
