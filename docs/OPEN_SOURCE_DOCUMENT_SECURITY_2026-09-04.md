...
A dedicated Warrantee-only Cloud Run deployment remains specified and tested in `services/clamav-scanner/cloud-run-config.mjs` as an unactivated future option. A segregated DigitalOcean alternative is prepared in `services/clamav-scanner/DIGITALOCEAN.md`; it defines a separate Compose project, private network, signature volume, root-only environment, host-loopback wrapper port, dedicated hostname, resource limits, and no document persistence or mounts from other workloads. The owner postponed ClamAV deployment on 5 September 2026, so neither runtime may be activated without renewed approval. The application supports a bounded `DOCUMENT_SECURITY_SCANNER_TIMEOUT_MS` for future remote scanner verification.

The Google Cloud project displayed `Start your Free Trial` on 5 September 2026. No API, service, repository, secret, or billing relationship was created. No DigitalOcean ClamAV containers, secrets, DNS, TLS, or production cutover were activated. The current internal scanner remains configured while ClamAV is postponed.
