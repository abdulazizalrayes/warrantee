...
- Protected owner report for regular browsers: `/en/owner/brief.txt` or `/ar/owner/brief.txt`
...
- As a signed-in Warrantee admin, request `/en/owner/brief.txt?days=30&limit=50&page=1` (or the Arabic locale path) to review categorical intents, partial-answer counts, locale/protocol breakdowns, improvement tags, and blocked-content categories. Question wording and hashes are not stored or displayed. This regular-browser path returns the protected JSON as inline plain text because some client-side filters block direct JSON-report navigation. It is explicitly marked `noindex, nofollow`; the JSON and API aliases remain available for authenticated integrations.
