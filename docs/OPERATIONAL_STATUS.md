...
- Production remains on `https://warrantee.io/api/internal/document-security-scan` with strict clean-before-download gating. Do not claim ClamAV is active until owner-approved Google activation plus health, clean-file, isolated EICAR, cold-start, log-privacy, CI, and Production Security Gate verification pass.
- Deployment, cutover, rollback, and identity-lock instructions are recorded in `services/clamav-scanner/CLOUD_RUN.md`.

## 2026-08-30 Account-Aware Pricing, Signup, Quotas, And Indexing

- Warrantee now distinguishes the two signup paths consistently: Personal is for individuals tracking their own purchases, while Business is for sellers and issuers managing customer warranties in a company workspace. Email/password and Google/Apple OAuth preserve the selected account type and business name through authentication.
- Personal Free includes 10 personal warranties and 1 user. Business Free includes the first 100 issued customer warranties and 1 business user. Professional is a founding launch offer displayed as `SAR 15 / USD 4 per month`, with 1,000 warranties and 3 users. Enterprise remains custom for higher-volume or custom-control requirements.
- Professional online checkout remains fail-closed while `STRIPE_PRO_PRICE_ID` is absent. The launch price is displayed for demand validation, but Warrantee must not claim that automatic paid activation is live until the postponed billing rollout is approved and verified.
- Production database migrations `20260830143000_account_aware_plans_and_warranty_quotas.sql` and `20260830151500_harden_business_onboarding_rpc.sql` are applied and recorded. Database-level warranty limits now cover browser, API, bulk-import, and ingestion paths; service callers receive a stable `409 warranty_limit_reached` response. The business-onboarding RPC is service-role only, and Supabase security advisors report zero findings.
- Commit `d469db9ca2cca918c046cd5c08f5db03df36f9b6` is deployed in production as `dpl_8NUeBFvtrs33EyTWApd5k1C6Q5YF` and aliased to `https://warrantee.io`. Main CI and Production Security Gates passed, including authenticated E2E, RLS, controlled-load, agent-readiness, and disposable-QA cleanup checks.
- Google Search Console was opened in regular Chrome under `abdulaziz.alrayes@gmail.com` for the `https://warrantee.io/` property. On August 30, `/en/pricing`, `/en/about`, and `/en/guide` each still reported `Crawled - currently not indexed`; Google confirmed crawl allowed, fetch successful, indexing allowed, sitemap discovery, and the correct self-canonical. Request Indexing succeeded for all three, and Google returned `Indexing requested`, adding each URL to the priority crawl queue. Indexing remains an asynchronous Google decision.
...
- **Warrantee identity lock:** Google Cloud, Google Search Console, and Sentry operations were performed under `abdulaziz.alrayes@gmail.com`. The Google Cloud project was independently matched as `Warrantee` / `warrantee-491217`; the Sentry organization/project were matched as `abdulazizalrayes` / `warrantee`; the Search Console property was `https://warrantee.io/`. Do not reuse these assets for another company.
- **Exposed optional Google Vision credential retired:** the dedicated `Warrantee Vision OCR` API key was confirmed to match the key retained in an old Sentry HTTP breadcrumb. The key was deleted from Google Cloud, and `GOOGLE_CLOUD_VISION_API_KEY` was removed from Warrantee Vercel and ignored local environment files. Mistral OCR 4.1 remains the verified production provider, so this removal does not reduce the active OCR service.
- **Future Google Vision calls are URL-safe:** PR `#8` merged as commit `a2bca65a147143415eb2807db4b641051d26fb15`. Optional Google Vision requests now use the clean `images:annotate` URL and send the credential through `x-goog-api-key`; a regression test forbids query-string API keys. Isolated verification passed 186 unit tests, type-check, lint, production build, and a zero-vulnerability `npm ci` audit.
- **Least-privilege Sentry inventory is active:** `SENTRY_ISSUES_READ_TOKEN` has only `event:read`, is stored in GitHub Actions and an ignored local environment file, is not installed in the Vercel production runtime, and is never printed by the inventory script. The first token was revoked immediately after a redaction-format mismatch was detected; only the replacement remains active.
- **Historical Sentry incident closed:** `WARRANTEE-4` (`Cannot find module 'node-fetch'` on `POST /api/ocr`) was resolved after its Tesseract output-tracing fix and the Vision credential hardening reached production. `npm run observability:sentry-issues -- --summary-only --fail-on-unresolved` now reports `unresolvedCount: 0`.
- **Production proof:** deployment `dpl_HS4dJ8KaZgDtM5iGcNTfrUqG1dhW` is `Ready`, targets production, and is aliased to `https://warrantee.io`. Custom-domain smoke and operational readiness passed. Main CI run `32596735128` passed. Manually dispatched Production Security Gates run `32597009447` passed smoke, agent Markdown audit, zero-unresolved Sentry inventory, anonymous RLS, operational readiness, authenticated E2E, controlled load, and disposable-QA cleanup. The previous known-good production deployment remains the rollback target if this release regresses.
- **Vercel email interpretation:** a Vercel bot email showing a PR deployment as `Building` is an informational point-in-time notification, not a failure. Verify the final deployment status and custom production domain before classifying it as an incident; PR `#8` preview and production deployments both completed successfully.
- **Current Search Console sample:** `/en/faq` is on Google. `/en/guide`, `/en/cookies`, `/en/about`, and `/en/pricing` currently report `Crawled - currently not indexed`. For all four, Google reports sitemap discovery, crawl allowed, successful fetch, indexing allowed, and the correct self-canonical. This is currently a Google selection/staleness outcome, not a robots, canonical, or deployment defect. These live results supersede older historical snapshots below while preserving those entries as dated evidence.
...
- **Sentry issue visibility is separated from release upload.** `npm run observability:sentry-issues` uses a least-privilege `SENTRY_ISSUES_READ_TOKEN` with only `event:read` and never prints it. The token is stored in GitHub Actions and an ignored local environment file, not in the Vercel runtime. The release token remains unchanged and must not be broadened for issue inventory.
