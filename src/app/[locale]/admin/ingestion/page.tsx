              ...
              return (
                <tr key={job.id} style={{
                  ...
                  <td style={{ padding: '12px 8px' }}>
                    <div style={{ fontWeight: 500 }}>{job.from_name || job.from_email}</div>
                    {job.from_name && <div style={{ fontSize: '12px', color: '#94A3B8' }}>{job.from_email}</div>}
                  </td>
                    ...
                    {job.ingestion_attachments?.map((a) => (
                      <div key={a.id} style={{ fontSize: '12px' }}>
                        ...
                        {a.aggregate_confidence != null && (
                          <span style={{ color: '#64748B' }}> ({Math.round(a.aggregate_confidence * 100)}%)</span>
                        )}
                      </div>
                    ))}
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ marginBottom: '16px' }}>
              <strong>{isRtl ? 'المرسل: ' : 'From: '}</strong>{selectedJob.from_email}<br/>
              <strong>{isRtl ? 'الموضوع: ' : 'Subject: '}</strong>{selectedJob.subject || '—'}<br/>
              <strong>{isRtl ? 'الثقة: ' : 'Trust: '}</strong>
                ...
                {selectedJob.fraud_signals.filter((f) => !f.resolved).map((f) => (
                  <div key={f.id} style={{
                    ...
                    <span style={{ fontSize: '13px' }}>
                      ...
                      <span style={{ color: '#94A3B8', marginInlineStart: '8px' }}>({f.severity})</span>
                    </span>
                  </div>
                ))}
              </div>
            ...
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end', flexWrap: 'wrap' }}>
              ...
              <button
                onClick={() => handleResolve(selectedJob.id, 'approve')}
                disabled={resolving}
                style={{
                  padding: '10px 20px', borderRadius: '8px', border: 'none',
              ...
              >
                {resolving ? (isRtl ? 'جارٍ...' : 'Processing...') : (isRtl ? 'قبول' : 'Approve')}
              </button>
            </div>
          </div>
        </div>
